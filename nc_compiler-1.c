...
void compile(FILE *in, FILE *out, int is_lib) {
    ...
    while (fgets(line, MAX_LINE, in)) {
        ...
        // Controle de fluxo
        if (starts_with(trimmed,"!if!"))   { fprintf(out,"%sif(%s){\n",pad,trim(trimmed+4)); indent++; continue; }
        if (starts_with(trimmed,"!elif!")) { indent--; fprintf(out,"%s}else if(%s){\n",pad,trim(trimmed+6)); indent++; continue; }
...
int main(int argc, char **argv) {
    ...
    if(ret==0){
        if(is_lib) printf("✓ Biblioteca: %s.o\n",outname);
        else printf("✓ Compilado: %s\n",outname);
